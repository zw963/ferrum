...
module Ferrum
  class Node
    ...
    getter :page, :target_id, :node_id, :description, :tag_name
    ...
    def click(*, mode = :left, keys = [] of Symbol, offset = {} of BlackHole => BlackHole, delay = 0)
    ...
    def computed_style
      page
        ...
        .each_with_object({} of BlackHole => BlackHole) { |style, memo| memo[style["name"]] = style["value"] }
    end

    private def bounding_rect_coordinates
      evaluate <<-JS
    ...
    private def content_quads
      quads = page.command("DOM.getContentQuads", nodeId: node_id)["quads"]
      raise CoordinatesNotFoundError.new "Node is either not visible or not an HTMLElement" if quads.size.zero?

      quads
    end

    private def content_quads_with(*, delay = MOVING_WAIT_DELAY)
    ...
    private def get_position(points, offset_x, offset_y, position)
      x = y = nil
