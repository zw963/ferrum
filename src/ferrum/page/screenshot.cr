...
module Ferrum
  class Page
    module Screenshot
      ...
      private def save_file(path, data)
      ...
      private def common_options(*, encoding = :base64, path = nil)
        encoding = encoding.to_sym
        encoding = :binary if path
        [path, encoding]
      end

      private def pdf_options(**opts)
        format = opts.delete(:format)
        options = DEFAULT_PDF_OPTIONS.merge(opts)

        if format
          if opts[:paper_width]? || opts[:paper_height]
            raise ArgumentError.new "Specify :format or :paper_width, :paper_height"
          end
      ...
      private def screenshot_options(path = nil, *, format = nil, scale = 1.0, **options)
        screenshot_options = {} of Symbol => String
      ...
      private def format_options(format, path, quality)
      ...
      private def area_options(full, selector, scale)
      ...
      private def bounding_rect(selector)
        rect = evaluate_async(%(
          const rect = document
      ...
      private def to_camel_case(option)
      ...
      private def capture_screenshot(options, full, background_color)
      ...
      private def maybe_resize_fullscreen(full)
        if full
      ...
      private def with_background_color(color)
        if color
          raise ArgumentError.new "Accept Ferrum::RGBA class only" unless color.is_a?(RGBA)
