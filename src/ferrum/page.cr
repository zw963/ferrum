...
require "./network"
require "./page/frames"
require "./page/screenshot"
require "./page/animation"
require "./page/tracing"
require "./page/stream"
...
module Ferrum
  class Page
    ...
    delegate at_css, at_xpath, css, xpath,
      ...
      add_script_tag, add_style_tag, to: @main_frame
    ...
    property :referrer, :timeout
    getter :target_id, :browser, :event, :tracing
    ...
    def initialize(target_id, browser, *, proxy)
    ...
    def go_to(url = nil)
      ...
      response = command("Page.navigate", wait: GOTO_WAIT, params: options)
    ...
    def set_window_bounds(bounds = {} of String => String)
      @browser.command("Browser.setWindowBounds", params: {windowId: window_id, bounds: bounds})
    end

    def command(method, *, wait = 0, slowmoable = false, params = {} of Symbol => String)
    ...
    private def prepare_page
      ...
      if @browser.options.save_path
        unless Pathname.new(@browser.options.save_path).absolute?
          raise Error.new "supply absolute path for `:save_path` option"
        end
    ...
    private def history_navigate(delta)
    ...
    private def combine_url!(url_or_path)
    ...
    private def proxy=(options)
      @proxy_host = options.try &.[](:host) || @browser.options.proxy.try &.[](:host)
      @proxy_port = options.try &.[](:port) || @browser.options.proxy.try &.[](:port)
      @proxy_user = options.try &.[](:user) || @browser.options.proxy.try &.[](:user)
      @proxy_password = options.try &.[](:password) || @browser.options.proxy.try &.[](:password)
    end
  end
end
