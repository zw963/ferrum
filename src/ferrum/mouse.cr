module Ferrum
  class Mouse
    ...
    # @return [self]
    #
    def click(*, x, y, delay = 0, wait = CLICK_WAIT, options = {} of BlackHole => BlackHole)
      move(x: x, y: y)
      down(**options)
      sleep(delay)
    ...
    # @return [self]
    #
    def down(*, options = {} of BlackHole => BlackHole)
      tap { mouse_event(type: "mousePressed", options: options) }
    end
    ...
    # @return [self]
    #
    def up(*, options = {} of BlackHole => BlackHole)
      tap { mouse_event(type: "mouseReleased", options: options) }
    end
    ...
    private def mouse_event(*, type, button = :left, count = 1, modifiers = nil, wait = 0)
      ...
      @page.command("Input.dispatchMouseEvent", wait: wait, slowmoable: true, params: options)
    end

    private def validate_button(button)
      button = button.to_s
      raise "Invalid button: #{button}" unless VALID_BUTTONS.includes?(button)
