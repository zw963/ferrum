...
module Ferrum
  class Keyboard
    ...
    #
    def down(key)
      key = normalize_keys(Array.wrap(key)).first
      type = key[:text] ? "keyDown" : "rawKeyDown"
      @page.command("Input.dispatchKeyEvent", slowmoable: true, type: type, params: key)
      self
    end
    ...
    def up(key)
      key = normalize_keys(Array.wrap(key)).first
      @page.command("Input.dispatchKeyEvent", slowmoable: true, type: "keyUp", params: key)
      self
    end
    ...
    #
    def type(*keys)
      keys = normalize_keys(Array.wrap(keys))

      keys.each do |key|
        type = key[:text] ? "keyDown" : "rawKeyDown"
        @page.command("Input.dispatchKeyEvent", type: type, params: key)
        @page.command("Input.dispatchKeyEvent", slowmoable: true, type: "keyUp", params: key)
      end
    ...
    private def normalize_keys(keys, pressed_keys = [] of BlackHole, memo = [] of BlackHole)
      case keys
      when Array
        raise ArgumentError.new "empty keys passed" if keys.empty?

        pressed_keys.push([] of BlackHole)
      ...
      when String
        ...
        keys.each_char.map do |char|
          key = KEYS[char]? || {} of BlackHole => BlackHole
        ...
        raise ArgumentError.new "unexpected argument"
      end
    end
    ...
    private def combine_strings(keys)
      keys
    ...
    private def to_options(hash)
      hash.inject({} of BlackHole => BlackHole) { |memo, (k, v)| memo.merge({k.to_sym => v}) }
    end
  end
end
