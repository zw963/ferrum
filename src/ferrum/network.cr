require "./network/exchange"
require "./network/intercepted_request"
require "./network/auth_request"
...
module Ferrum
  class Network
    ...
    getter :traffic
    @exchange : Exchange
    @blacklist : Array(BlackHole)
    @whitelist : Array(BlackHole)

    def initialize(@page)
      @traffic = [] of BlackHole
      @exchange = nil
      @blacklist = nil
      @whitelist = nil
    end
    ...
    #
    def clear(type)
      raise ArgumentError.new ":type should be in #{CLEAR_TYPE}" unless CLEAR_TYPE.includes?(type)
    ...
    def intercept(*, pattern = "*", resource_type = nil, request_stage = nil, handle_auth_requests = true)
      ...
      if resource_type && RESOURCE_TYPES.none?(resource_type.to_sym)
        raise ArgumentError.new "Unknown resource type '#{resource_type}' must be #{RESOURCE_TYPES.join(" | ")}"
      end

      if request_stage && REQUEST_STAGES.none?(request_stage.to_sym)
        raise ArgumentError.new "Unknown request stage '#{request_stage}' must be #{REQUEST_STAGES.join(" | ")}"
      end
    ...
    def authorize(*, user, password, type = :server, &block)
      raise ArgumentError.new AUTHORIZE_TYPE_WRONG unless AUTHORIZE_TYPE.includes?(type)
      raise ArgumentError.new AUTHORIZE_BLOCK_MISSING if !block && !@page.subscribed?("Fetch.requestPaused")

      @authorized_ids ||= {} of Symbol => BlackHole
      @authorized_ids[type] ||= [] of BlockHole
    ...
    def emulate_network_conditions(*, offline = false, latency = 0,
      ...
      params[:connectionType] = connection_type if connection_type && ALLOWED_CONNECTION_TYPE.includes?(connection_type)

      @page.command("Network.emulateNetworkConditions", params: params)
      true
    end
    ...
    private def subscribe_request_will_be_sent
      @page.on("Network.requestWillBeSent") do |params|
        ...
        exchange = select_method(request.id).last
        exchange = build_exchange(request.id) unless exchange.try &.blank?
        ...
        if params["redirectResponse"]
          previous_exchange = select_method(request.id)[-2]
    ...
    private def subscribe_response_received
      @page.on("Network.responseReceived") do |params|
        exchange = select_method(params["requestId"]).last
    ...
    private def subscribe_loading_finished
      @page.on("Network.loadingFinished") do |params|
        response = select_method(params["requestId"]).last.try &.response
    ...
    private def subscribe_loading_failed
    ...
    private def subscribe_log_entry_added
      @page.on("Log.entryAdded") do |params|
        entry = params["entry"]? || {} of BlackHole => BlackHole
        if entry["source"] == "network" && entry["level"] == "error"
          exchange = select_method(entry["networkRequestId"]).last
          exchange.error ||= Network::Error.new
    ...
    private def blacklist_subscribe
      return unless blacklist?
      raise ArgumentError.new "You can't use blacklist along with whitelist" if whitelist?
    ...
    private def whitelist_subscribe
      return unless whitelist?
      raise ArgumentError.new "You can't use whitelist along with blacklist" if blacklist?
